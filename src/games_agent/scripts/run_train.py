...
def main(cfg):
    ...
    for i_episode in range(num_episodes):
        ...
        for t in count():
            ...
            dqn_agent.memory.push(state, action, next_state, reward, terminated)
                ...
                if n_step % 10 == 0:
                    ...
                    for key in target_net_state_dict.keys():
                        writer.add_histogram(
                            "TargetNet/" + key, target_net_state_dict[key], n_step
                        )
                    for key in policy_net_state_dict.keys():
                        writer.add_histogram(
                            "PolicyNet/" + key, policy_net_state_dict[key], n_step
                        )
